...
// Header presence detects standard duration support, not platform clock support.
// Define to 0 to disable, or to 1 on toolchains without __has_include whose
// standard library is known to provide <chrono>.
...
/// and final values must be representable. Compact storage requires a subsequent
/// explicit SmallDuration conversion. No time-point/clock conversion is implied.
template <typename Rep, typename Period>
constexpr Duration FromChrono(std::chrono::duration<Rep, Period> value) {
  return Micros(std::chrono::duration_cast<std::chrono::microseconds>(value).count());
}
